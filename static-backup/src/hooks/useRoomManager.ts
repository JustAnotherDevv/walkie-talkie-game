...
const useRoomManagerStore = create<RoomManagerState>((set, get) => ({
  ...
  setAllRoomsUnlocked: (unlocked) => {
    const state = get();
    const updated = state.rooms.map((room, index) => {
      if (room.isUnlocked === unlocked) return room;
      if (unlocked) {
        state.onDoorUnlocked.forEach(cb => cb(index));
      }
      return { ...room, isUnlocked: unlocked };
    });
    set({ rooms: updated });
  },
  ...
  sealAllRooms: () => {
    const state = get();
    set({ rooms: state.rooms.map(room => ({ ...room, isUnlocked: false })) });
  },
}));
...
export function useRoomManager() {
  ...
  return {
    ...
    isDoorUnlocked: store.isDoorUnlocked,
    setAllRoomsUnlocked: store.setAllRoomsUnlocked,
    sealAllRooms: store.sealAllRooms,
    canEnterRoom,
    getGatingPuzzleId,
